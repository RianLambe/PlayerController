...
[CustomEditor(typeof(CompleteCharacterController))]
public class CharacterControllerEditor : Editor
{
    ...
    SerializedProperty autoFootstepSounds;
    SerializedProperty autoJumpSounds;
    SerializedProperty autoLandSounds;
    ...
    private void OnEnable() {
        ...
        autoFootstepSounds = serializedObject.FindProperty("autoFootstepSounds");
        autoJumpSounds = serializedObject.FindProperty("autoJumpSounds");
        autoLandSounds = serializedObject.FindProperty("autoLandSounds");
    ...
    public override void OnInspectorGUI() {
        ...
        CompleteCharacterController controller = (CompleteCharacterController)target;
        ...
        if(jumpSettingsDD) {
            controller.jumpMode = (CompleteCharacterController.jumpModes)EditorGUILayout.EnumPopup(new GUIContent("Jump mode", "Changes the style of jumping between various modes."), controller.jumpMode);

            switch (controller.jumpMode) {
                case CompleteCharacterController.jumpModes.Standard:
                    controller.jumpHeight = EditorGUILayout.FloatField(new GUIContent("Jump height", "This is the height that the player will jump. Note that due to the way the physics system works within unity the player will always be slightly off this height."), controller.jumpHeight);
                    break;

                case CompleteCharacterController.jumpModes.Charge:
                ...
                case CompleteCharacterController.jumpModes.Hold:
                    controller.jumpHeight = EditorGUILayout.FloatField(new GUIContent("Jump power", "This is the force applied to the player every frame."), controller.jumpHeight);
                    break;
            }

            controller.maxJumps = EditorGUILayout.IntField(new GUIContent("Max jumps", "If this number is more than one the player will be able to complete that amount of jumps before landing again. If this value is set to 1 then it will enable 'Coyote time'."), controller.maxJumps);
            ...
            controller.jumpBufferMode = (CompleteCharacterController.jumpBufferModes)EditorGUILayout.EnumPopup(new GUIContent("Jump buffer", "When jump buffer is enabled, the player will be able to press the jump button before landing and it will cache the jump to be used straight away after landing."), controller.jumpBufferMode);
